...
#                                                  p_tax,p_rate,p_type,p_unit,p_cell,p_term,p_payment,p_plant,p_brand)   #FUN-D10004 add ,p_brand
...
# Modify.........: No:FUN-C20077 13/04/23 By jt_chen 原取價取核價檔判斷ima44,ima908的改為判斷pmj15、pmj16
# Modify.........: No:TQC-D50106 13/05/22 By jt_chen 因FUN-C20077回傳多筆資料導致,需抓最大筆
# Modify.........: No:FUN-D10004 13/06/13 By jt_chen 取價時pmh_file需增加判斷廠牌
# Modify.........: No:MOD-DB0137 13/11/20 By Vampire 小數位數須調整為跨營運中心寫法
# Modify.........: No:MOD-DC0005 13/12/03 By Vampire FUNCTION s_defprice_A12()中計算含稅單價前,先將未稅單價作取位
# Modify.........: No:MOD-DC0032 13/12/06 By Vampire 需考慮單位轉換。
# Modify.........: No:MOD-DC0125 13/12/19 By Vampire l_pnq03='A3'不重新取價
# Modify.........: No:MOD-DC0155 13/12/25 By Summer 調整依單價含稅否回推含稅未稅單價 
# Modify.........: No:MOD-E10002 14/01/02 By SunLM s_defprice_A4应该可以跨库访问
# Modify.........: No:MOD-FC0022 15/12/03 By fionchend 執行s_defprice_new時,一開始應先清空g_errno,避免造成之前的值殘留,而判斷錯誤
...
DEFINE gr_par RECORD 
       ...
       plant   LIKE azp_file.azp01,   #FUN-D10004 add ,
       brand   LIKE pmh_file.pmh07    #FUN-D10004 add
       END RECORD
...
#FUNCTION s_defprice_new(p_part,p_vender,p_curr,p_date,p_qty,p_task,p_tax,p_rate,p_type,p_unit,p_cell,p_term,p_payment,p_plant) #No.MOD-9C0258   #FUN-D10004 mark
FUNCTION s_defprice_new(p_part,p_vender,p_curr,p_date,p_qty,p_task,p_tax,p_rate,p_type,p_unit,p_cell,p_term,p_payment,p_plant,p_brand)           #FUN-D10004 add
...
DEFINE p_brand    LIKE pmh_file.pmh07             #FUN-D10004 add
...
DEFINE l_sql      STRING              #MOD-DB0137 add 

  #SELECT azi04 INTO t_azi04               #MOD-B10164
  #MOD-DB0137 mark start -----
  ...
  LET g_errno = ' '      #MOD-FC0022 add
   ...
   IF cl_null(p_cell) THEN
      LET p_cell = ' '
   END IF
   IF cl_null(p_brand) THEN LET p_brand = ' ' END IF   #FUN-D10004 add
   ...
   LET gr_par.brand = p_brand  #FUN-D10004 add

  #MOD-DB0137 add start -----
   LET l_sql = " SELECT azi03,azi04 FROM ",cl_get_target_table(gr_par.plant,'azi_file'),
               "  WHERE azi01 = '",p_curr,"'" #幣別
   CALL cl_replace_sqldb(l_sql) RETURNING l_sql
   CALL cl_parse_qry_sql(l_sql,gr_par.plant) RETURNING l_sql
   PREPARE azi_p FROM l_sql
   EXECUTE azi_p INTO t_azi03,t_azi04 
   IF cl_null(t_azi03) THEN LET t_azi03 = g_azi03 END IF
   IF cl_null(t_azi04) THEN LET t_azi04 = g_azi04 END IF
  #MOD-DB0137 add end   -----
...
#DEFINE l_ima44      LIKE ima_file.ima44    #FUN-C20077 mark
#DEFINE l_ima908     LIKE ima_file.ima908   #FUN-C20077 mark
DEFINE l_pmj15      LIKE pmj_file.pmj15     #FUN-C20077 add
DEFINE l_pmj16      LIKE pmj_file.pmj15     #FUN-C20077 add
DEFINE l_unitrate   LIKE pmn_file.pmn121
DEFINE l_sw         LIKE type_file.num10
DEFINE l_sql        STRING              
DEFINE l_sql1       STRING                  #FUN-C20077 add
DEFINE l_sql2       STRING                  #FUN-C20077 add
#FUN-A80104----start----
...
#FUN-A80104----end----
DEFINE l_gec07      LIKE gec_file.gec07 #MOD-DC0155 add
   ...
   #FUN-C20077  -- mark start --
   #LET l_sql = "SELECT ima44,ima908 FROM ",cl_get_target_table(gr_par.plant,'ima_file'), 
   #           " WHERE ima01 = ?"                                                         
   #CALL cl_replace_sqldb(l_sql) RETURNING l_sql              #FUN-A50102		       		
   #CALL cl_parse_qry_sql(l_sql,gr_par.plant) RETURNING l_sql #FUN-A50102              
   ...
   #                  INTO l_pmj15,l_pmj16                                                
   #FUN-C20077 -- mark end --
   #FUN-C20077 -- add start --
   LET l_sql1 = "SELECT pmj15,pmj16 "
   LET l_sql2 = "  FROM ",cl_get_target_table(gr_par.plant,'pmj_file'),",",
                          cl_get_target_table(gr_par.plant,'pmi_file'),    
                " WHERE pmi01=pmj01 ",
   ...
   LET l_sql1=l_sql1 CLIPPED," ORDER BY pmj09 DESC"   #TQC-D50106 add
   CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1              
   CALL cl_parse_qry_sql(l_sql1,gr_par.plant) RETURNING l_sql1 
   PREPARE pmj15_p1 FROM l_sql1
  #EXECUTE pmj15_p1 INTO l_pmj15,l_pmj16   #TQC-D50106 mark 
  #TQC-D50106 -- add start --
   DECLARE pmj15_cur CURSOR FOR pmj15_p1   
   OPEN pmj15_cur
   FETCH pmj15_cur INTO l_pmj15,l_pmj16 
   CLOSE pmj15_cur
  #TQC-D50106 -- add end --
   #FUN-C20077 -- add end --
   #IF cl_null(gr_par.unit) THEN LET gr_par.unit=l_ima44 END IF                           #FUN-C20077 mark 
    IF cl_null(gr_par.unit) THEN LET gr_par.unit=l_pmj15 END IF                           #FUN-C20077 add
    IF g_sma.sma116 MATCHES '[13]' THEN   
      #LET l_ima44 = l_ima908                                                             #FUN-C20077 mark
       LET l_pmj15 = l_pmj16                                                              #FUN-C20077 add
    END IF 
    LET l_unitrate = 1
   #IF NOT cl_null(gr_par.unit) AND gr_par.unit <> l_ima44 THEN                           #FUN-C20077 mark
    IF NOT cl_null(gr_par.unit) AND gr_par.unit <> l_pmj15 THEN                           #FUN-C20077 add
      #CALL s_umfchk(gr_par.part,gr_par.unit,l_ima44)                                     #FUN-C20077 mark
       CALL s_umfchk(gr_par.part,gr_par.unit,l_pmj15)                                     #FUN-C20077 add
...
#FUN-A80104---start---       
    ...
    FOREACH pnq04_cur INTO l_pnq04
       ...
       IF l_pnq04 IS NULL THEN CONTINUE FOREACH END IF
         ...
         #  #MOD-DC0032 mark start -----
         #  #LET l_price=l_price*l_unitrate
         #  #LET lt_price=lt_price*l_unitrate  #TQC-C40042 mark #MOD-D30015 remove # 
         #  #MOD-DC0032 mark end   -----
         #  #MOD-DC0032 add start -----
         #   LET l_price =cl_digcut(l_price*l_unitrate ,t_azi03)
         #   LET lt_price=cl_digcut(lt_price*l_unitrate,t_azi03) 
         #  #MOD-DC0032 add end   -----
         #  #MOD-D30015 add start ------
         #  #IF l_pnq03 = 'A4' THEN                   #MOD-DC0125 mark
         #   IF l_pnq03 = 'A4' OR l_pnq03 = 'A3' THEN #MOD-DC0125 add
         ...
         #MOD-DC0155 mark --end--
          ...
          ELSE
             #取值时未包含币种的判断，需再行计算汇率
             LET l_price =cl_digcut(l_price*l_unitrate/l_exrate,t_azi03)
             LET lt_price=cl_digcut(lt_price*l_unitrate/l_exrate,t_azi03)
          END IF
          #依單價含稅否回推含稅未稅單價
          SELECT gec07 INTO l_gec07 FROM gec_file
           WHERE gec01 = gr_par.tax
             AND gec011 = '1'
          IF l_gec07 = 'Y' THEN
             LET l_price = cl_digcut(lt_price/(1+gr_par.rate/100),t_azi03)
          ELSE
             LET lt_price =cl_digcut(l_price * (1 + gr_par.rate/100),t_azi03)
          END IF
          #MOD-DC0155 add --end--
...
DEFINE l_sql STRING
   ...
   END IF
   LET l_price = cl_digcut(l_price,t_azi03) #MOD-DC0005 add
...
DEFINE l_ima151 LIKE ima_file.ima151  #TQC-C30005 add
  ...
  #EXECUTE pmh_sel USING gr_par.part,gr_par.vender,gr_par.curr,                          #FUN-D10004 mark
   EXECUTE pmh_sel USING gr_par.part,gr_par.vender,gr_par.brand,gr_par.curr,             #FUN-D10004 add
...
DEFINE l_sql3    STRING  #MOD-E10002 add
...
#    END IF
#MOD-E10002 mark end-------------------- 
#MOD-E10002 add begin------------------
    IF l_pmi05 = 'Y' THEN          #分量計價
       LET l_sql3 = " SELECT pmr05,pmr05t FROM ",cl_get_target_table(gr_par.plant,'pmr_file'),
                    " WHERE pmr01='",l_pmi01,"'",
                    " AND pmr02 ='",l_pmj02,"'",
                    " AND ",gr_par.qty," BETWEEN pmr03 AND pmr04 ",
                    " ORDER BY pmr05 "
       PREPARE pmr05_pre FROM l_sql3
       DECLARE pmr05_cur CURSOR FOR pmr05_pre
       FOREACH pmr05_cur INTO l_j07r05,lt_j07r05   
           IF NOT cl_null(l_j07r05) THEN
               EXIT FOREACH
           END IF

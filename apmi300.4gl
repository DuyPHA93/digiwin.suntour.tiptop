...
# Modify.........: No.MOD-G50072 16/05/12 By fionchen 調整MOD-C40212,當入庫的收貨單為JIT收貨時,稅別須由JIT收貨單取得 
# Modify.........: No.06100389_M001_6_1 23/12/28 By DSC.ANHDUY add column MGD色號 pmnud02,品名 ima02a,規格 ima021a
...
DEFINE 
   ...
   g_rvv   DYNAMIC  ARRAY OF RECORD        #程式變數(Program Variables)
            ...
            pmnud02 LIKE pmn_file.pmnud02, #MGD色號 #06100389_M001_6_1 add
            ima02a  LIKE ima_file.ima02,   #品名    #06100389_M001_6_1 add
            ima021a LIKE ima_file.ima021,  #規格    #06100389_M001_6_1 add
   ...
   g_rvv_t          RECORD                 #程式變數 (舊值)
            ...
            pmnud02 LIKE pmn_file.pmnud02,  #06100389_M001_6_1 add
            ima02a  LIKE ima_file.ima02,    #06100389_M001_6_1 add
            ima021a LIKE ima_file.ima021,   #06100389_M001_6_1 add
...
MAIN
   ...
   OPEN WINDOW t300_w WITH FORM "cpm/42f/apmi300"
...
DEFINE   l_n1           LIKE type_file.num5       #No.FUN-BB0086
   ...
   LET g_forupd_sql = " SELECT rvv02,rvv36,rvv37,rvv31,' ',' ','',' ',' ',",     #06100389_M001_6_1 add '',' ',' '
...
#MOD-C40212 add begin(JIT無採購單收貨,取不到採購單號的值)
                  IF cl_null(g_rvv[l_ac].rvv36) THEN #先從pmh_file抓取稅別,稅率
                     #MOD-G50072 add start ---------------------------
                     #若無採購單資料,則取收貨單,若該收貨單為JIT收貨單,則稅別資料由JIT收貨單取得
                     SELECT rva115,rva116,azi04,azi03
                       INTO g_rvv[l_ac].pmm21,g_rvv[l_ac].pmm43,t_azi04,t_azi03
                       FROM rvv_file,rva_file,azi_file
                      WHERE rvv04 = rva01
                        AND rvv01 = g_rvv01
                        AND rvv02 = g_rvv[l_ac].rvv02
                        AND rva00 = '2'      #JIT收貨
                        AND rva113 = azi01
                     IF cl_null(g_rvv[l_ac].pmm21) THEN
                     #MOD-G50072 add end   ---------------------------
...
#MOD-C40212 add end
   ...
   #                 SELECT pmm21,pmm43,azi04 INTO g_rvv[l_ac].pmm21,g_rvv[l_ac].pmm43,t_azi04      #MOD-A80052        #MOD-B800
                     ...
                     #No.FUN-610018
                     #06100389_M001_6_1 add --begin--
                     SELECT pmnud02 INTO g_rvv[l_ac].pmnud02
                      FROM pmn_file
                      WHERE pmn01 = g_rvv[l_ac].rvv36 AND pmn02 = g_rvv[l_ac].rvv37
                     #06100389_M001_6_1 add --end--
                  END IF    #MOD-C40212 add
                  #06100389_M001_6_1 add --begin--
                  IF NOT cl_null(g_rvv[l_ac].pmnud02) THEN
                     SELECT ima02,ima021
                       INTO g_rvv[l_ac].ima02a,g_rvv[l_ac].ima021a
                       FROM ima_file 
                      WHERE ima01 = g_rvv[l_ac].pmnud02    #MGD color no.
                  END IF
                  #06100389_M001_6_1 add --end--               
                 #MOD-C10082---add---str---
...
#              LET g_rvv[l_ac].rvv38 = cl_digcut(g_rvv[l_ac].rvv38,g_azi03)          #MOD-B80063 mark
      ...
      AFTER FIELD pmnud02
         IF NOT cl_null(g_rvv[l_ac].pmnud02) THEN 
            CALL i300_pmnud02(g_rvv[l_ac].pmnud02)
            IF NOT cl_null(g_errno) THEN
               CALL cl_err(g_rvv[l_ac].pmnud02,g_errno,0)
               NEXT FIELD pmnud02
            END IF
         END IF
      #06100389_M001_6_1 add --end--
      ...
      ON ACTION CONTROLP
         CASE
            WHEN INFIELD(rvv86) #計價單位
               ...
               NEXT FIELD rvv86
            WHEN INFIELD(pmnud02) #計價單位
               CALL cl_init_qry_var()
               LET g_qryparam.form ="q_ima"
               LET g_qryparam.where ="imaud10 = '1'"
               LET g_qryparam.default1 = g_rvv[l_ac].pmnud02
               CALL cl_create_qry() RETURNING g_rvv[l_ac].pmnud02
               DISPLAY BY NAME g_rvv[l_ac].pmnud02
               NEXT FIELD pmnud02
            OTHERWISE EXIT CASE
         END CASE
      #CHI-A80025 add --end--
...
DEFINE  l_rvu02  LIKE rvu_file.rvu02          #MOD-B40012
  ...
  #            "   FROM rvv_file,ima_file,OUTER pmm_file",
   LET g_sql = " SELECT rvv02,rvv36,rvv37,rvv31,'', ", 
               "        '','','','',rvv17,rvv86,rvv87,rvv38,rvv38t,rvv39,rvv39t,pmm21,pmm43,'' ",  #06100389_M001_6_1 add '','',''
  ...
  #MOD-B30669 mod -end--
   ...
   FOREACH rvv_curs INTO g_rvv[g_cnt].*   #單身 ARRAY 填充
      ...
      #MOD-B30669 add --end--
      #06100389_M001_6_1 add --begin--
      SELECT pmnud02 INTO g_rvv[g_cnt].pmnud02
       FROM pmn_file
       WHERE pmn01 = g_rvv[g_cnt].rvv36 AND pmn02 = g_rvv[g_cnt].rvv37
      IF NOT cl_null(g_rvv[g_cnt].pmnud02) THEN
         SELECT ima02,ima021 INTO g_rvv[g_cnt].ima02a,g_rvv[g_cnt].ima021a
           FROM ima_file
          WHERE ima01 = g_rvv[g_cnt].pmnud02
      END IF
      #06100389_M001_6_1 add --end--
...
#06100389_M001_6_1 add --begin--
FUNCTION i300_pmnud02(p_pmnud02)
DEFINE
      p_pmnud02  LIKE pmn_file.pmnud02,
     ...
     SELECT ima01,ima02,ima021,imaacti INTO l_ima01,l_ima02,l_ima021,l_imaacti
       FROM ima_file
      WHERE ima01 = p_pmnud02 AND imaud10 = '1'
     CASE
     ...
     IF cl_null(g_errno) THEN
        LET g_rvv[l_ac].ima02a = l_ima02
        LET g_rvv[l_ac].ima021a = l_ima021
        DISPLAY BY NAME g_rvv[l_ac].ima02a,g_rvv[l_ac].ima021a
     END IF
END FUNCTION
#06100389_M001_6_1 add --end--